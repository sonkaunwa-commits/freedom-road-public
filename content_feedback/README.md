...
Evidence validation rejects booleans and all non-finite numeric values (`NaN`, `Infinity`, `-Infinity`) for policy rates and metric numerators/denominators. Observation and source collection timestamps must be timezone-aware ISO-8601 values, and a source cannot claim collection after the enclosing observation timestamp. This keeps arithmetic and provenance ordering deterministic.
