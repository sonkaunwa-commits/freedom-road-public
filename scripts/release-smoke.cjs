...
function validateConfig(config) {
  ...
  if (!Array.isArray(config.checks) || config.checks.length === 0) throw new Error('config.checks must be a non-empty array');
  const checks = config.checks;
  const name = config.name === undefined ? 'release-smoke' : requireString(config.name, 'name');
  ...
  if (config.timeoutMs !== undefined) {
    if (!Number.isInteger(config.timeoutMs) || config.timeoutMs <= 0) throw new Error('timeoutMs must be a positive integer');
  ...
  const normalized = checks.map((raw, index) => {
    ...
    const id = raw.id === undefined ? `check-${index + 1}` : requireString(raw.id, `checks[${index}].id`);
    ...
    if (hasPath === hasUrl) throw new Error(`${id} must declare exactly one of path or url`);

    let status = 200;
    if (raw.status !== undefined) {
      if (!Number.isInteger(raw.status) || raw.status < 100 || raw.status > 599) throw new Error(`${id}.status must be an integer HTTP status between 100 and 599`);
      status = raw.status;
    }

    let contentTypeContains = null;
    if (raw.contentTypeContains !== undefined) contentTypeContains = requireString(raw.contentTypeContains, `${id}.contentTypeContains`);
  ...
  return { name, baseUrl: baseUrl ? baseUrl.toString() : null, timeoutMs, checks: normalized };
}
...
async function fetchWithTimeout(url, timeoutMs, fetchImpl) {
  ...
  try {
    return await fetchImpl(url, {
      ...
      headers: { 'cache-control': 'no-cache', pragma: 'no-cache', 'user-agent': 'freedom-road-release-smoke/1.0' },
    });
...
async function runOne(check, options) {
  ...
  try {
    ...
    assertions.push({ type: 'status', expected: check.status, actual: response.status, pass: response.status === check.status });
    for (const text of check.contains) assertions.push({ type: 'contains', expected: text, pass: body.includes(text) });
    for (const text of check.notContains) assertions.push({ type: 'not_contains', expected: text, pass: !body.includes(text) });
    if (check.contentTypeContains) assertions.push({ type: 'content_type_contains', expected: check.contentTypeContains, actual: contentType, pass: contentType.toLowerCase().includes(check.contentTypeContains.toLowerCase()) });
    return { id: check.id, url: check.url.toString(), final_url: response.url || targetUrl.toString(), status: response.status, content_type: contentType, pass: assertions.every(item => item.pass), duration_ms: Date.now() - started, assertions };
  } catch (error) {
    return { id: check.id, url: check.url.toString(), pass: false, duration_ms: Date.now() - started, error: error && error.name === 'AbortError' ? `timeout after ${options.timeoutMs}ms` : String(error && error.message ? error.message : error), assertions };
  }
}
...
async function runSmoke(rawConfig, runtime = {}) {
  ...
  const checks = [];
  for (const check of config.checks) checks.push(await runOne(check, { timeoutMs: config.timeoutMs, fetchImpl, cacheBustToken }));
  return { schema_version: SCHEMA_VERSION, name: config.name, checked_at: new Date().toISOString(), base_url: config.baseUrl, pass: checks.every(item => item.pass), duration_ms: Date.now() - started, checks };
}

function formatTextReport(result) {
  const lines = [`release-smoke ${result.pass ? 'PASS' : 'FAIL'} name=${result.name}`, `checked_at=${result.checked_at}`, `duration_ms=${result.duration_ms}`];
  for (const check of result.checks) {
    ...
    for (const assertion of check.assertions || []) if (!assertion.pass) lines.push(`  assertion_failed type=${assertion.type} expected=${JSON.stringify(assertion.expected)} actual=${JSON.stringify(assertion.actual ?? null)}`);
  }
  return lines.join('\n');
}
...
if (require.main === module) main().catch(error => { console.error(`release-smoke ERROR: ${error && error.message ? error.message : error}`); process.exit(2); });
