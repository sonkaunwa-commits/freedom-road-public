...
async function lofExplanation(page,name){
 await page.evaluate(()=>{const q=(window.SEC_QUESTIONS||[]).find(x=>x.knowledge==='LOF'&&x.quizEligible!==false&&x.strict!==false&&x.type==='single');if(!q)throw new Error('LOF single missing');window.__V46_LOF_STEM=q.q;});
 await page.locator('[data-start-sub="finance"]').click();await page.waitForSelector('.questionCard');
 for(let n=0;n<120;n++){
  const isLof=await page.evaluate(()=>document.querySelector('.questionCard h1')?.textContent.trim()===window.__V46_LOF_STEM);
  if(isLof)break;
  await page.locator('#mainAction').click().catch(()=>{}); await page.waitForTimeout(20);
  if(n===119)throw new Error(`${name}: could not reach LOF via practice`);
 }
 const meta=await page.evaluate(()=>{const text=document.querySelector('.questionCard h1')?.textContent.trim();const q=(window.SEC_QUESTIONS||[]).find(x=>String(x.q||'').trim()===text);return q?{count:q.o.length,wrong:q.o.map((_,i)=>i).find(i=>!q.a.includes(i))??0,stem:q.q,opts:q.o}:null});
 assert(meta&&!metaRx.test(meta.stem),`${name}: LOF stem is meta-learning`);assert(!meta.opts.some(v=>/期限.*收益率.*完全相同|政策利率.*只影响央行/.test(v)),`${name}: unrelated LOF distractor remains`);
 ...
 assert(await page.locator('.v451OptionRow').count()===meta.count,`${name}: every option must have rationale`);const reasons=await page.locator('.v451OptionRow>span').allInnerTexts();assert(reasons.every(x=>x.trim().length>=18),`${name}: shallow rationale`);
 assert(!/美食|探店|空镜头|ND滤镜|CPL滤镜|摄影|相机/.test(t),`${name}: unrelated course content leaked into explanation`);
 assert(await page.locator('.questionCard .option.v451Correct').count()>=1,`${name}: correct option not highlighted`);assert(await page.locator('.questionCard .option.v451Wrong').count()>=1,`${name}: wrong option not highlighted`);
}
async function caseQuality(page,name){
 const data=await page.evaluate(()=>{const q=(window.SEC_QUESTIONS||[]).find(x=>x.type==='comprehensive'&&x.quizEligible!==false&&x.strict!==false&&String(x.caseStem||'').length>=65);return q?{stem:q.q,caseStem:q.caseStem,oa:q.oa,o:q.o}:null});
 assert(data,`${name}: long comprehensive case missing`);assert(data.caseStem.length>=65,`${name}: case material too short`);assert(Array.isArray(data.oa)&&data.oa.length===data.o.length,`${name}: case option rationales missing`);
}
async function mockReview(page,name){
 await page.locator('#backBtn').click();await page.waitForSelector('.v45Dashboard');await page.locator('[data-tab="mock"]').click();await page.waitForSelector('.v45PaperList');await page.locator('[data-paper]').first().click();await page.waitForSelector('.questionCard');
 await page.locator('#paletteBtn').click();await page.waitForSelector('#submitMockFromPalette');await page.locator('#submitMockFromPalette').click();await page.waitForSelector('#confirmSubmit');await page.locator('#confirmSubmit').click();await page.waitForSelector('#mockReview');await page.locator('#mockReview').click();await page.waitForSelector('.v451Deep',{timeout:12000});const t=await page.locator('.v451Deep').innerText();assert(t.includes('你的答案：未作答')&&t.includes('正确答案')&&t.includes('逐项拆解')&&t.includes('本题知识卡'),`${name}: mock review explanation incomplete`);
}
async function run(type,name,opts){const b=await type.launch({headless:true}),c=await b.newContext(opts),p=await c.newPage();const errors=[];p.on('pageerror',e=>errors.push(e.message));await open(p,name);await bankQuality(p,name);await caseQuality(p,name);await lofExplanation(p,name);await mockReview(p,name);assert(!errors.length,`${name}: ${errors.join(' | ')}`);console.log(`${name} PASS v4.6 authentic bank + deep explanation + mock review`);await b.close();}
