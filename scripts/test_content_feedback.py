...
def expect_error(record=None, policy=None):
    try:
        evaluate_feedback(record if record is not None else deepcopy(SAMPLE), policy if policy is not None else deepcopy(POLICY))
...
for observed_at in ("2026-09-03T00:00:00", "not-a-time"):
    bad = deepcopy(SAMPLE)
    bad["observed_at"] = observed_at
    expect_error(bad)
