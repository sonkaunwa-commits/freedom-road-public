...
function stat(f){
  ...
  let eta='';
  if(ref>=near[0]&&ref<=near[1]&&trend.includes('向上')) eta='今天已经接近可考虑的位置，下午2:20左右再确认一次。';
  else if(trend==='偏弱'||ref<stop) eta='现在不按日期抄底。先等走势重新稳定，至少连续几天不再创新低再看。';
  else if(distanceToBuy>0&&avgMove){const d=Math.max(1,Math.ceil(distanceToBuy/Math.max(.25,avgMove))),lo=Math.max(1,Math.floor(d*.6)),hi=Math.max(lo+1,Math.ceil(d*1.6));eta=`按最近波动速度粗略看，可能还要等约 ${lo}～${hi} 个交易日才有机会靠近更舒服的位置；这只是估算，不是预测日期。`;}
  ...
  return {last,ref,m20,m60,m120,r20,r60,r120,dd,dev60,trend,near,deep,stop,action,tone,why,how,avgMove,eta,maxdd250,vol60,prob120,prob250};
}
...
function budgetPlan(budget,action){if(!budget||budget<=0)return '';let first=Math.round(budget*.25/100)*100,second=first,reserve=budget-first-second;if(action.includes('不要')||action.includes('不买')||action.includes('不追')||action.includes('等')||action.includes('停'))return `如果你最多准备投入 ${Number(budget).toLocaleString()} 元：今天先买 0 元，钱先留着；真正到合适位置时，第一笔大约 ${first.toLocaleString()} 元，第二笔再约 ${second.toLocaleString()} 元，至少留 ${reserve.toLocaleString()} 元备用。`;return `如果你最多准备投入 ${Number(budget).toLocaleString()} 元：今天先用大约 ${first.toLocaleString()} 元试第一笔；后面位置更合适再用约 ${second.toLocaleString()} 元，至少保留 ${reserve.toLocaleString()} 元，不一次买满。`;}
