(()=>{
  ...
  function enhance(){const c=market();
    ...
    document.querySelectorAll('.btn.add').forEach(b=>setText(b,/已设为|已加入/.test(b.textContent)?'✓ 已设为我的关注':'⭐ 点这里设为我的关注'));
    document.querySelectorAll('.btn.del').forEach(b=>setText(b,'删除这只'));
    document.querySelectorAll('.btn.delw').forEach(b=>setText(b,'移出关注'));
    document.querySelectorAll('.answer .a1').forEach(el=>setText(el,c.open?`今日建议 · ${dateFull}`:`休市日参考 · ${dateFull}`));
    document.querySelectorAll('.answer .a3 b').forEach(el=>{if(/今天怎么做|下一交易日怎么做|\d+月\d+日怎么做/.test(el.textContent))setText(el,c.open?`${dateShort}怎么做：`:'下一交易日怎么做：')});
  }
  ...
  function addVersion(){if(document.querySelector('.ui-version'))return;const state=document.querySelector('#state');if(!state)return;const v=document.createElement('div');v.className='ui-version';v.textContent=`界面版本 ${UI_VERSION}`;state.insertAdjacentElement('afterend',v)}
  ...
  let pending=false;function run(){if(pending)return;pending=true;requestAnimationFrame(()=>{pending=false;addVersion();addGuide();enhance();markClickable()})}run();new MutationObserver(run).observe(document.body,{childList:true,subtree:true});
})();
