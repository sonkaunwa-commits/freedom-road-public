...
const mechanical=/(考生|复习时|做题时|答题时|考试常把|最应记住|有助于正确理解|无需结合具体条件判断|忽略题干中的主体|所有产品、渠道和业务场景中均适用完全相同的规则|不受任何市场规则约束|仅由单一因素决定|无需考虑任何具体条件即可)/;
...
function rewriteBanking(q,n){
 ...
 {stem:'某银行吸收1000万元企业存款，同时向另一企业发放600万元贷款，并为客户提供一项尚未履约的付款担保。下列分类正确的是？',opts:['存款属于资产业务，贷款属于负债业务，担保属于表内资产业务。','存款属于负债业务，贷款属于资产业务，担保通常属于表外业务。','存款和贷款都属于负债业务，担保不形成任何风险。','存款、贷款和担保在业务分类上完全相同。'],ans:[1],why:['错误。三类业务的经济实质不同：存款是资金来源，贷款是资金运用，担保可能形成未来责任。','正确。站在银行角度，存款形成偿还义务，贷款形成债权，尚未履约的担保通常作为表外业务管理。','错误。贷款通常形成资产；而担保即使暂未履约，也可能产生未来信用和资金风险。','错误。业务分类正是常见辨析点，不能因为都涉及资金或客户就归为同一类。']}
 ];
 apply(q,sets[n%sets.length]);
}
let bankN=0;for(const q of B)if(bankTopic(q))rewriteBanking(q,bankN++);
let quarantined=0;
for(const q of B){if(q.bankPolishV461)continue;const text=[q.q,...(q.o||[])].join(' ');if(q.authenticityV46&&mechanical.test(text)){q.strict=false;q.quizEligible=false;q.authenticityIssue='mechanical-generated-wording-v461';q.quality='quarantined-v4.6.1';quarantined++;}}
window.SEC_QUESTIONS=B;window.SEC_V461_BANK_POLISH={version:'4.6.1',bankingRewritten:bankN,mechanicalQuarantined:quarantined,policy:'targeted-authenticity-polish-no-framework-change'};
})();