...
function validCoursePoint(d,p){const txt=[d.part,d.chapter_guess,d.overview,p.topic,p.explanation,p.teacher_angle,p.exam_focus,p.memory_tip,p.trap,...(p.keywords||[])].join(' ');const noise=/(美食|探店|拍摄|空镜头|镜头选择|滤镜|摄影|相机|蓝调时刻|黄金时刻|景别|焦段|ND滤镜|CPL滤镜|画面表现)/;const securities=/(证券|股票|债券|基金|金融|市场|交易|投资|期货|期权|衍生|风险|利率|收益率|估值|央行|货币|融资|合规|法律|法规|保荐|承销|适当性|内幕|操纵|信息披露|公司治理|从业)/;return !noise.test(txt)&&securities.test(txt)}
async function loadCourses(){for(const [s,url] of [['finance','../exam-live/bilibili-finance-learning-v1.json'],['law','../exam-live/bilibili-law-learning-v1.json']]){try{const r=await fetch(url+'?v=20260828c',{cache:'no-store'});if(!r.ok)continue;const j=await r.json();for(const d of j.documents||[])for(const p of d.points||[]){if(!validCoursePoint(d,p))continue;courseCards.push({s,part:d.part||'',chapter:d.chapter_guess||'',bvid:d.bvid||'',page:d.page||1,topic:p.topic||'',explanation:p.explanation||'',teacherAngle:p.teacher_angle||'',examFocus:p.exam_focus||'',memoryTip:p.memory_tip||'',trap:p.trap||'',keywords:p.keywords||[]})}}catch(_){}}}
...
function relatedQuestions(q){
 const eligible=bank().filter(x=>x.id!==q.id&&x.quizEligible!==false&&x.strict!==false);
 const same=eligible.filter(x=>x.s===q.s&&q.knowledge&&x.knowledge===q.knowledge);
 const near=eligible.filter(x=>x.s===q.s&&x.ch===q.ch&&(!q.knowledge||x.knowledge!==q.knowledge));
 const rows=[...same.map(x=>({x,rel:'同知识点'})),...near.map(x=>({x,rel:'同章迁移'}))].slice(0,4);
 if(!rows.length)return '';
 return `<section class="v451Block"><h3>关联考题 · 换一种考试问法</h3><p class="v451Lead">用于确认同一知识点换成另一种题干、选项或情境后仍然能够判断。</p>${rows.map(({x,rel})=>`<div class="v451Related"><b>${esc(rel)} · ${esc(typeLabel(x.type))} · ${esc(x.knowledge||x.ch||'')}</b><span>${esc(x.q)}</span></div>`).join('')}</section>`
}
...
window.SEC_QUIZ_V451={version:VERSION,features:['learning-first-deep-explanation','option-specific-rationale','wrong-answer-diagnosis','knowledge-card','related-transfer-questions','course-links-quality-filtered','source-provenance','mock-review-explanation']};
})();