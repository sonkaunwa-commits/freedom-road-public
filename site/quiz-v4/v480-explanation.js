...
function syncVersionLabel(){
  document.querySelectorAll('.statLine b').forEach(el=>{if(/^v4\.5\.0$/.test((el.textContent||'').trim()))el.textContent='v'+VERSION;});
  document.querySelectorAll('.meCard').forEach(card=>{if((card.textContent||'').includes('版本')){card.querySelectorAll('b').forEach(el=>{if(/^v4\.5\.0$/.test((el.textContent||'').trim()))el.textContent='v'+VERSION;});}});
}
