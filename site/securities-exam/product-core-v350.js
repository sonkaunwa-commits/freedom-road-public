...
function settings(raw={}){
  return {
    targetDate: raw.targetDate||'2026-09-19',
    dailyMinutes: clamp(Number(raw.dailyMinutes)||60,30,120),
    studyDays: clamp(Number(raw.studyDays)||6,4,7),
    reminderTime: raw.reminderTime||'20:30',
    bufferDay: Number.isInteger(+raw.bufferDay)?+raw.bufferDay:0,
  };
}
function buildPlan(input={}){
  const s=settings(input.settings||{}), now=input.now||new Date(), days=daysLeft(s.targetDate,now), p=phase(days);
  const dailyDone=Math.max(0,+input.dailyDone||0), wrong=Math.max(0,+input.wrongCount||0), due=Math.max(0,+input.dueCount||0);
  const learned=Math.max(0,+input.learnedToday||0), reviewed=Math.max(0,+input.reviewedToday||0), hasSession=!!input.hasSession;
  const isBuffer=now.getDay()===s.bufferDay && s.studyDays<7;
  let learnMin,practiceMin,reviewMin;
  ...
  const learnGoal=isBuffer?3:clamp(Math.round(learnMin/3),4,12);
  const practiceGoal=isBuffer?0:clamp(Math.round(practiceMin/1.15),10,p.id==='sprint'?45:40);
  const reviewGoal=clamp(Math.max(due,Math.min(wrong,Math.round(reviewMin/1.2))),isBuffer?3:5,20);
  const tasks=[];
  if(isBuffer){
  ...
  const shouldMock=!isBuffer && (p.id==='mock'||p.id==='sprint') && (now.getDay()===3||now.getDay()===6);
...
function nextInterval(level='again',stage=0){
  if(level==='again')return {stage:0,days:1};
  if(level==='hard')return {stage:Math.max(0,stage),days:[1,2,4,7,14][Math.min(stage,4)]};
  const ns=Math.min(5,stage+1); return {stage:ns,days:[1,3,7,14,30,60][ns]||60};
}
window.SEC_PRODUCT_CORE_V350={VERSION,dayKey,daysLeft,phase,settings,buildPlan,nextInterval};
})();