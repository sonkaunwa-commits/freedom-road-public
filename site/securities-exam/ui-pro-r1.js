...
function boot(){
 ...
 $$('.tabs .tab').forEach(x=>{x.setAttribute('aria-label',x.textContent.trim());x.setAttribute('type','button')});
 ...
 $$('select').forEach(x=>x.setAttribute('aria-label',x.closest('.field')?.querySelector('label')?.textContent||'选择项'));
}
